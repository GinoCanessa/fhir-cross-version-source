...
| ------ | ------ | ------ | ------- |
| Encounter | Encounter | Equivalent | R5 `Encounter` maps as Equivalent to R4 `Encounter` |
...
| Encounter.class | Encounter.class | RelatedTo | R5 `Encounter.class` maps as RelatedTo to R4 `Encounter.class` - class made the element mandatory; class increased the minimum cardinality from 0 to 1; class changed from array to scalar (max cardinality from * to 1); class changed the binding strength from Preferred to Extensible; class has change due to type change: R5 class CodeableConcept has no equivalent or mapped type in R4 class |
...
| Encounter.diagnosis.use | Encounter.diagnosis.use | RelatedTo | R5 `Encounter.diagnosis.use` maps as RelatedTo to R4 `Encounter.diagnosis.use` - use changed from array to scalar (max cardinality from * to 1) |
...
| Encounter.language | Encounter.language | RelatedTo | R5 `Encounter.language` maps as RelatedTo to R4 `Encounter.language` - language changed the binding strength from Required to Preferred |
...
| Encounter.status | Encounter.status | RelatedTo | R5 `Encounter.status` maps as RelatedTo to R4 `Encounter.status` - status has INCOMPATIBLE required binding for code type: http://hl7.org/fhir/ValueSet/encounter-status|5.0.0 and http://hl7.org/fhir/ValueSet/encounter-status|4.0.1 |
