Comparison of hl7.fhir.r5.core#5.0.0 and hl7.fhir.r4.core#4.0.1
Generated at Thursday, July 25, 2024 3:56:50 PM

| Side | Url | Name | Title | Description |
| --- | --- | --- | --- | --- |
| Source | http://hl7.org/fhir/ValueSet/account-status | AccountStatus | Account Status | Indicates whether the account is available to be used. |
| Target | http://hl7.org/fhir/ValueSet/account-status | AccountStatus | AccountStatus | Indicates whether the account is available to be used. |


Comparison Result: Equivalent


### Mapping details

| Status | Count |
| ------ | ----- |
Equivalent | 5 |


| Source | Target | Status | Message |
| ------ | ------ | ------ | ------- |
| active | active | Equivalent | R5 `active` is equivalent to R4 `active`. |
| entered-in-error | entered-in-error | Equivalent | R5 `entered-in-error` is equivalent to R4 `entered-in-error`. |
| inactive | inactive | Equivalent | R5 `inactive` is equivalent to R4 `inactive`. |
| on-hold | on-hold | Equivalent | R5 `on-hold` is equivalent to R4 `on-hold`. |
| unknown | unknown | Equivalent | R5 `unknown` is equivalent to R4 `unknown`. |

