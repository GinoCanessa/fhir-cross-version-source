Comparison of 
Generated at Friday, April 4, 2025 2:58:59 PM

### EnteralFormulaAdditiveTypeCode

|      |     |
| ---: | --- |
| Package | hl7.fhir.r5.core@5.0.0 |
| VS Name | EnteralFormulaAdditiveTypeCode |
| Canonical URL | `http://hl7.org/fhir/ValueSet/entformula-additive` |
| Version | 5.0.0 |
| Description | EnteralFormulaAdditiveType: Codes for the type of modular component such as protein, carbohydrate or fiber to be provided in addition to or mixed with the base formula. This value set is provided as a suggestive example. |
| Status | `Draft` |
| Has Escape Valve Code | `False` |
| Database Key | `4537` |
| Database Concept Count | `5` |
| Database Active Concept Count | `5` |
### Bindings

| Source | Element | Binding | Strength | Element Short |
| ------ | ------- | ------- | -------- | ------------- |
| `http://hl7.org/fhir/StructureDefinition/NutritionOrder` | `NutritionOrder.enteralFormula.additive.type` | `http://hl7.org/fhir/ValueSet/entformula-additive` | `Example` | Type of modular component to add to the feeding |

### Referenced Systems

* `http://terminology.hl7.org/CodeSystem/entformula-additive`
### Empty Projection

This Value Set resulted in no projection (no mappings to other packages).

### Codes

| System | Code | Display |
| ------ | ---- | ------- |
| `http://terminology.hl7.org/CodeSystem/entformula-additive` | `carbohydrate` | Carbohydrate |
| `http://terminology.hl7.org/CodeSystem/entformula-additive` | `fiber` | Fiber |
| `http://terminology.hl7.org/CodeSystem/entformula-additive` | `lipid` | Lipid |
| `http://terminology.hl7.org/CodeSystem/entformula-additive` | `protein` | Protein |
| `http://terminology.hl7.org/CodeSystem/entformula-additive` | `water` | Water |
