Comparison of 
Generated at Friday, April 4, 2025 2:58:58 PM

### SNOMEDCTDrugTherapyStatusCodes

|      |     |
| ---: | --- |
| Package | hl7.fhir.r5.core@5.0.0 |
| VS Name | SNOMEDCTDrugTherapyStatusCodes |
| Canonical URL | `http://hl7.org/fhir/ValueSet/reason-medication-status-codes` |
| Version | 5.0.0 |
| Description | This value set includes some taken and not taken reason codes from SNOMED CT - provided as an exemplar |
| Status | `Draft` |
| Has Escape Valve Code | `` |
| Database Key | `4831` |
| Database Concept Count | `0` |
| Database Active Concept Count | `0` |
### Bindings

| Source | Element | Binding | Strength | Element Short |
| ------ | ------- | ------- | -------- | ------------- |
| `http://hl7.org/fhir/StructureDefinition/MedicationStatement` | `MedicationStatement.adherence.reason` | `http://hl7.org/fhir/ValueSet/reason-medication-status-codes` | `Example` | Details of the reason for the current use of the medication |

### Expansion Failure

Failed to expand this value set: Expansion is limited
