...
| **`unconfirmed`**| _Equivalent_ <br/>(25/11989)| `unconfirmed`| | | | | | | 
| **`confirmed`**| →→→→ _SourceIsBroaderThanTarget_ →→→→ <br/>(30)<hr/>←←←← _Equivalent_ ←←←← <br/>(11990) | `confirmed`| | | | | | | 
| **`active`**| | | | | | | | | 
| **`resolved`**| | | | | | | | | 
