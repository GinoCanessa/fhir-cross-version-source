Comparison of 
Generated at Tuesday, April 1, 2025 1:39:06 PM

### Observation Category Codes

|      |     |
| ---: | --- |
| Package | hl7.fhir.r2.core@1.0.2 |
| VS Name | Observation Category Codes |
| Canonical URL | `http://hl7.org/fhir/ValueSet/observation-category` |
| Version | 1.0.2 |
| Description | Observation Category codes. |
| Status | `Draft` |
| Has Escape Valve Code | `False` |
| Database Key | `253` |
| Database Concept Count | `8` |
| Database Active Concept Count | `8` |
### Bindings

| Source | Element | Binding | Strength | Element Short |
| ------ | ------- | ------- | -------- | ------------- |
| `http://hl7.org/fhir/StructureDefinition/Observation` | `Observation.category` | `http://hl7.org/fhir/ValueSet/observation-category` | `Example` | Classification of  type of observation |

### Referenced Systems

* `http://hl7.org/fhir/observation-category`
### Empty Projection

This Value Set resulted in no projection (no mappings to other packages).

### Codes

| System | Code | Display |
| ------ | ---- | ------- |
| `http://hl7.org/fhir/observation-category` | `exam` | Exam |
| `http://hl7.org/fhir/observation-category` | `imaging` | Imaging |
| `http://hl7.org/fhir/observation-category` | `laboratory` | Laboratory |
| `http://hl7.org/fhir/observation-category` | `procedure` | Procedure |
| `http://hl7.org/fhir/observation-category` | `social-history` | Social History |
| `http://hl7.org/fhir/observation-category` | `survey` | Survey |
| `http://hl7.org/fhir/observation-category` | `therapy` | Therapy |
| `http://hl7.org/fhir/observation-category` | `vital-signs` | Vital Signs |
