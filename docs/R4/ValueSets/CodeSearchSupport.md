Comparison of 
Generated at Monday, April 14, 2025 6:17:27 PM

### CodeSearchSupport

|      |     |
| ---: | --- |
| Package | hl7.fhir.r4.core@4.0.1 |
| VS Name | CodeSearchSupport |
| Canonical URL | `http://hl7.org/fhir/ValueSet/code-search-support` |
| Version | 4.0.1 |
| Description | The degree to which the server supports the code search parameter on ValueSet, if it is supported. |
| Status | `Draft` |
| Has Escape Valve Code | `False` |
| Database Key | `2273` |
| Database Concept Count | `2` |
| Database Active Concept Count | `2` |
### Bindings

| Source | Element | Binding | Strength | Element Short |
| ------ | ------- | ------- | -------- | ------------- |
| `http://hl7.org/fhir/StructureDefinition/TerminologyCapabilities` | `TerminologyCapabilities.codeSearch` | `http://hl7.org/fhir/ValueSet/code-search-support\|4.0.1` | `Required` | explicit \| all |

### Referenced Systems

* `http://hl7.org/fhir/code-search-support`
### Mapping Table

| R2 | Comparison | R3 | Comparison | R4 | Comparison | R4B | Comparison | R5
| --- | --- | --- | --- | --- | --- | --- | --- | ---
| | | | | [CodeSearchSupport](/docs/R4/ValueSets/CodeSearchSupport.md)<br/> `http://hl7.org/fhir/ValueSet/code-search-support\|4.0.1` | →→→→→→→<br/>`Equivalent`<br/>- DBKey: `1417`<br/>- Reviewed: `n/a`<br/>- By: `n/a`<br/>- Identical: `False`<br/>→→→→→→→<hr/>←←←←←←←<br/>`Equivalent`<br/>- DBKey: `1418`<br/>- Reviewed: `n/a`<br/>- By: `n/a`<br/>- Identical: `False`<br/>←←←←←←←| [CodeSearchSupport](/docs/R4B/ValueSets/CodeSearchSupport.md)<br/> `http://hl7.org/fhir/ValueSet/code-search-support\|4.3.0` | →→→→→→→<br/>`SourceIsNarrowerThanTarget`<br/>- DBKey: `1002`<br/>- Reviewed: `n/a`<br/>- By: `n/a`<br/>- Identical: `False`<br/>→→→→→→→<hr/>←←←←←←←<br/>`SourceIsBroaderThanTarget`<br/>- DBKey: `1263`<br/>- Reviewed: `n/a`<br/>- By: `n/a`<br/>- Identical: `False`<br/>←←←←←←←| [CodeSearchSupport](/docs/R5/ValueSets/CodeSearchSupport.md)<br/> `http://hl7.org/fhir/ValueSet/code-search-support\|5.0.0` 

### Code Mappings


#### Map Group 0

This group is centered on the Value Set CodeSearchSupport from hl7.fhir.r4.core@4.0.1 (R4, key 3).
All codes from this value set are listed while other value sets only show contents that have relationships with those codes.

| *No Map* | Relationship | *No Map* | Relationship | R4 CodeSearchSupport| Relationship | [R4B CodeSearchSupport](/docs/R4B/ValueSets/CodeSearchSupport.md)| Relationship | [R5 CodeSearchSupport](/docs/R5/ValueSets/CodeSearchSupport.md)
| --- | --- | --- | --- | --- | --- | --- | --- | ---
| <td colspan="8">**R4** System: `http://hl7.org/fhir/code-search-support`
| | | | | **`explicit`**| _Equivalent_ <br/>(14664/14665)| `explicit`| _Equivalent_ <br/>(9449/11785)| `in-compose`
| | | | | **`all`**| _Equivalent_ <br/>(14666/14667)| `all`| _Equivalent_ <br/>(9448/11787)| `in-compose-or-expansion`
| | | | | *2 of 2 codes used* | | *2 of 2 codes used* | | *2 of 3 codes used* <br/>remaining codes:<br/>`in-expansion`

