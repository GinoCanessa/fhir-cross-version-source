...
| `PaymentNotice.identifier` | `PaymentNotice.identifier` | `identifier` | PaymentNotice.identifier | Business Identifier for the payment noctice | 0..* | Identifier |  |  |
...
| `PaymentNotice.language` | `PaymentNotice.language` | `language` | Resource.language | Language of the resource content | 0..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/all-languages` |
...
| `PaymentNotice.payment` | `PaymentNotice.payment` | `payment` | PaymentNotice.payment | Payment reference | 1..1 | Reference(http://hl7.org/fhir/StructureDefinition/PaymentReconciliation) |  |  |
...
| `PaymentNotice.provider` | `PaymentNotice.provider` | `provider` | PaymentNotice.provider | Responsible practitioner | 0..1 | Reference(http://hl7.org/fhir/StructureDefinition/Organization), Reference(http://hl7.org/fhir/StructureDefinition/Practitioner), Reference(http://hl7.org/fhir/StructureDefinition/PractitionerRole) |  |  |
...
| `PaymentNotice.status` | `PaymentNotice.status` | `status` | PaymentNotice.status | active \| cancelled \| draft \| entered-in-error | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/fm-status|4.3.0` |
...
This group is centered on the Structure Definition PaymentNotice from hl7.fhir.r4b.core@4.3.0 (R4B, key 4).
All elements from this structure are listed while other structures only show contents that have relationships with those elements.

| [R2 PaymentNotice](/docs/R2/Resources/PaymentNotice.md)| Relationship | [R3 PaymentNotice](/docs/R3/Resources/PaymentNotice.md)| Relationship | [R4 PaymentNotice](/docs/R4/Resources/PaymentNotice.md)| Relationship | R4B PaymentNotice| Relationship | [R5 PaymentNotice](/docs/R5/Resources/PaymentNotice.md)
| --- | --- | --- | --- | --- | --- | --- | --- | ---
...
| `PaymentNotice.provider`| →→→→ _SourceIsNarrowerThanTarget_ →→→→ <br/>(7064)<hr/>←←←← _SourceIsBroaderThanTarget_ ←←←← <br/>(7065)| `PaymentNotice.provider`| →→→→ _RelatedTo_ →→→→ <br/>(16915)<hr/>←←←← _RelatedTo_ ←←←← <br/>(1621)| `PaymentNotice.provider`| _Equivalent_<br/>(31400/31401)| **`PaymentNotice.provider`**| _Equivalent_<br/>(1917/2155)| `PaymentNotice.reporter`
