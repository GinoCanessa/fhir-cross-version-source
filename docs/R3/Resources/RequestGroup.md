...
| `RequestGroup.action` | `RequestGroup.action` | `action` |  | Proposed actions, if any | 0..* | BackboneElement |  |  |
| `RequestGroup.action.action` | `RequestGroup.action.action` | `action` |  | Sub action | 0..* | RequestGroup.action |  |  |
| `RequestGroup.action.cardinalityBehavior` | `RequestGroup.action.cardinalityBehavior` | `cardinalityBehavior` |  | single \| multiple | 0..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/action-cardinality-behavior` |
| `RequestGroup.action.code` | `RequestGroup.action.code` | `code` |  | Code representing the meaning of the action or sub-actions | 0..* | CodeableConcept |  |  |
| `RequestGroup.action.condition` | `RequestGroup.action.condition` | `condition` |  | Whether or not the action is applicable | 0..* | BackboneElement |  |  |
| `RequestGroup.action.condition.description` | `RequestGroup.action.condition.description` | `description` |  | Natural language description of the condition | 0..1 | string |  |  |
| `RequestGroup.action.condition.expression` | `RequestGroup.action.condition.expression` | `expression` |  | Boolean-valued expression | 0..1 | string |  |  |
...
| `RequestGroup.action.condition.id` | `RequestGroup.action.condition.id` | `id` | Element.id | xml:id (or equivalent in JSON) | 0..1 | id |  |  |
| `RequestGroup.action.condition.kind` | `RequestGroup.action.condition.kind` | `kind` |  | applicability \| start \| stop | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/action-condition-kind` |
| `RequestGroup.action.condition.language` | `RequestGroup.action.condition.language` | `language` |  | Language of the expression | 0..1 | string |  |  |
| `RequestGroup.action.condition.modifierExtension` | `RequestGroup.action.condition.modifierExtension` | `modifierExtension` | BackboneElement.modifierExtension | Extensions that cannot be ignored | 0..* | Extension |  |  |
| `RequestGroup.action.description` | `RequestGroup.action.description` | `description` |  | Short description of the action | 0..1 | string |  |  |
| `RequestGroup.action.documentation` | `RequestGroup.action.documentation` | `documentation` |  | Supporting documentation for the intended performer of the action | 0..* | RelatedArtifact |  |  |
...
| `RequestGroup.action.groupingBehavior` | `RequestGroup.action.groupingBehavior` | `groupingBehavior` |  | visual-group \| logical-group \| sentence-group | 0..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/action-grouping-behavior` |
| `RequestGroup.action.id` | `RequestGroup.action.id` | `id` | Element.id | xml:id (or equivalent in JSON) | 0..1 | id |  |  |
| `RequestGroup.action.label` | `RequestGroup.action.label` | `label` |  | User-visible label for the action (e.g. 1. or A.) | 0..1 | string |  |  |
| `RequestGroup.action.modifierExtension` | `RequestGroup.action.modifierExtension` | `modifierExtension` | BackboneElement.modifierExtension | Extensions that cannot be ignored | 0..* | Extension |  |  |
| `RequestGroup.action.participant` | `RequestGroup.action.participant` | `participant` |  | Who should perform the action | 0..* | Reference(http://hl7.org/fhir/StructureDefinition/Patient), Reference(http://hl7.org/fhir/StructureDefinition/Person), Reference(http://hl7.org/fhir/StructureDefinition/Practitioner), Reference(http://hl7.org/fhir/StructureDefinition/RelatedPerson) |  |  |
...
| `RequestGroup.note` | `RequestGroup.note` | `note` |  | Additional notes about the response | 0..* | Annotation |  |  |
| `RequestGroup.priority` | `RequestGroup.priority` | `priority` |  | routine \| urgent \| asap \| stat | 0..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/request-priority` |
| `RequestGroup.reason[x]` | `RequestGroup.reason[x]` | `reason[x]` |  | Reason for the request group | 0..1 | CodeableConcept, Reference(http://hl7.org/fhir/StructureDefinition/Resource) |  |  |
| `RequestGroup.replaces` | `RequestGroup.replaces` | `replaces` |  | Request(s) replaced by this request | 0..* | Reference(http://hl7.org/fhir/StructureDefinition/Resource) |  |  |
| `RequestGroup.status` | `RequestGroup.status` | `status` |  | draft \| active \| suspended \| cancelled \| completed \| entered-in-error \| unknown | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/request-status` |
| `RequestGroup.subject` | `RequestGroup.subject` | `subject` |  | Who the request group is about | 0..1 | Reference(http://hl7.org/fhir/StructureDefinition/Group), Reference(http://hl7.org/fhir/StructureDefinition/Patient) |  |  |
...
This group is centered on the Structure Definition RequestGroup from hl7.fhir.r3.core@3.0.2 (R3, key 2).
All elements from this structure are listed while other structures only show contents that have relationships with those elements.

| *No Map* | Relationship | R3 RequestGroup| Relationship | [R4 RequestGroup](/docs/R4/Resources/RequestGroup.md)| Relationship | [R4B RequestGroup](/docs/R4B/Resources/RequestGroup.md)| Relationship | [R5 RequestOrchestration](/docs/R5/Resources/RequestOrchestration.md)
| --- | --- | --- | --- | --- | --- | --- | --- | ---
...
| | | **`RequestGroup.definition`**| →→→→ _Equivalent_ →→→→ <br/>(32325)<hr/>←←←← _SourceIsBroaderThanTarget_ ←←←← <br/>(1657)| `base64Binary`| | | | | 
...
| | | **`RequestGroup.reason[x]`**| →→→→ _RelatedTo_ →→→→ <br/>(1272)<hr/>←←←← _RelatedTo_ ←←←← <br/>(1656)| `RequestGroup.reasonCode`| _Equivalent_<br/>(32375/32376)| `RequestGroup.reasonCode`| | | 
...
| | | **`RequestGroup.action.condition.description`**| →→→→ _SourceIsBroaderThanTarget_ →→→→ <br/>(1266)<hr/>←←←← _SourceIsBroaderThanTarget_ ←←←← <br/>(18001)| `RequestGroup.action.condition.expression`| _Equivalent_<br/>(32413/32414)| `RequestGroup.action.condition.expression`| | | 
| | | **`RequestGroup.action.condition.language`**| →→→→ _SourceIsBroaderThanTarget_ →→→→ <br/>(1267)<hr/>←←←← _SourceIsBroaderThanTarget_ ←←←← <br/>(18002)| `RequestGroup.action.condition.expression`| _Equivalent_<br/>(32413/32414)| `RequestGroup.action.condition.expression`| | | 
...
| | | *58 of 58 elements used* | | *58 of 60 elements used* | | *57 of 60 elements used* | | *0 of 95 elements used* 
