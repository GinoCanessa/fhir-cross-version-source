...
| -- | ---- | ---- | --------- | ----- | ----------- | ------------- | ---------------- | ----------------- |
| `GraphDefinition` | `GraphDefinition` | `GraphDefinition` | GraphDefinition | Definition of an graph of resources | 0..* | GraphDefinition |  |  |
| `GraphDefinition.contact` | `GraphDefinition.contact` | `contact` |  | Contact details for the publisher | 0..* | ContactDetail |  |  |
...
| `GraphDefinition.date` | `GraphDefinition.date` | `date` |  | Date this was last changed | 0..1 | dateTime |  |  |
| `GraphDefinition.description` | `GraphDefinition.description` | `description` |  | Natural language description of the graph definition | 0..1 | markdown |  |  |
| `GraphDefinition.experimental` | `GraphDefinition.experimental` | `experimental` |  | For testing purposes, not real usage | 0..1 | boolean |  |  |
...
| `GraphDefinition.jurisdiction` | `GraphDefinition.jurisdiction` | `jurisdiction` |  | Intended jurisdiction for graph definition (if applicable) | 0..* | CodeableConcept | `Extensible` | `http://hl7.org/fhir/ValueSet/jurisdiction` |
| `GraphDefinition.language` | `GraphDefinition.language` | `language` | Resource.language | Language of the resource content | 0..1 | code | `Extensible` | `http://hl7.org/fhir/ValueSet/languages` |
| `GraphDefinition.link` | `GraphDefinition.link` | `link` |  | Links this graph makes rules about | 0..* | BackboneElement |  |  |
| `GraphDefinition.link.description` | `GraphDefinition.link.description` | `description` |  | Why this link is specified | 0..1 | string |  |  |
...
| `GraphDefinition.link.id` | `GraphDefinition.link.id` | `id` | Element.id | xml:id (or equivalent in JSON) | 0..1 | id |  |  |
| `GraphDefinition.link.max` | `GraphDefinition.link.max` | `max` |  | Maximum occurrences for this link | 0..1 | string |  |  |
| `GraphDefinition.link.min` | `GraphDefinition.link.min` | `min` |  | Minimum occurrences for this link | 0..1 | integer |  |  |
| `GraphDefinition.link.modifierExtension` | `GraphDefinition.link.modifierExtension` | `modifierExtension` | BackboneElement.modifierExtension | Extensions that cannot be ignored | 0..* | Extension |  |  |
| `GraphDefinition.link.path` | `GraphDefinition.link.path` | `path` |  | Path in the resource that contains the link | 1..1 | string |  |  |
| `GraphDefinition.link.sliceName` | `GraphDefinition.link.sliceName` | `sliceName` |  | Which slice (if profiled) | 0..1 | string |  |  |
| `GraphDefinition.link.target` | `GraphDefinition.link.target` | `target` |  | Potential target for the link | 1..* | BackboneElement |  |  |
| `GraphDefinition.link.target.compartment` | `GraphDefinition.link.target.compartment` | `compartment` |  | Compartment Consistency Rules | 0..* | BackboneElement |  |  |
| `GraphDefinition.link.target.compartment.code` | `GraphDefinition.link.target.compartment.code` | `code` |  | Identifies the compartment | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/compartment-type` |
| `GraphDefinition.link.target.compartment.description` | `GraphDefinition.link.target.compartment.description` | `description` |  | Documentation for FHIRPath expression | 0..1 | string |  |  |
| `GraphDefinition.link.target.compartment.expression` | `GraphDefinition.link.target.compartment.expression` | `expression` |  | Custom rule, as a FHIRPath expression | 0..1 | string |  |  |
...
| `GraphDefinition.link.target.compartment.id` | `GraphDefinition.link.target.compartment.id` | `id` | Element.id | xml:id (or equivalent in JSON) | 0..1 | id |  |  |
| `GraphDefinition.link.target.compartment.modifierExtension` | `GraphDefinition.link.target.compartment.modifierExtension` | `modifierExtension` | BackboneElement.modifierExtension | Extensions that cannot be ignored | 0..* | Extension |  |  |
| `GraphDefinition.link.target.compartment.rule` | `GraphDefinition.link.target.compartment.rule` | `rule` |  | identical \| matching \| different \| custom | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/graph-compartment-rule` |
...
| `GraphDefinition.link.target.id` | `GraphDefinition.link.target.id` | `id` | Element.id | xml:id (or equivalent in JSON) | 0..1 | id |  |  |
| `GraphDefinition.link.target.link` | `GraphDefinition.link.target.link` | `link` |  | Additional links from target resource | 0..* | GraphDefinition.link |  |  |
| `GraphDefinition.link.target.modifierExtension` | `GraphDefinition.link.target.modifierExtension` | `modifierExtension` | BackboneElement.modifierExtension | Extensions that cannot be ignored | 0..* | Extension |  |  |
| `GraphDefinition.link.target.profile` | `GraphDefinition.link.target.profile` | `profile` |  | Profile for the target resource | 0..1 | uri |  |  |
| `GraphDefinition.link.target.type` | `GraphDefinition.link.target.type` | `type` |  | Type of resource this link refers to | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/resource-types` |
...
| `GraphDefinition.name` | `GraphDefinition.name` | `name` |  | Name for this graph definition (computer friendly) | 1..1 | string |  |  |
| `GraphDefinition.profile` | `GraphDefinition.profile` | `profile` |  | Profile on base resource | 0..1 | uri |  |  |
| `GraphDefinition.publisher` | `GraphDefinition.publisher` | `publisher` |  | Name of the publisher (organization or individual) | 0..1 | string |  |  |
| `GraphDefinition.purpose` | `GraphDefinition.purpose` | `purpose` |  | Why this graph definition is defined | 0..1 | markdown |  |  |
| `GraphDefinition.start` | `GraphDefinition.start` | `start` |  | Type of resource at which the graph starts | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/resource-types` |
| `GraphDefinition.status` | `GraphDefinition.status` | `status` |  | draft \| active \| retired \| unknown | 1..1 | code | `Required` | `http://hl7.org/fhir/ValueSet/publication-status` |
...
| `GraphDefinition.url` | `GraphDefinition.url` | `url` |  | Logical URI to reference this graph definition (globally unique) | 0..1 | uri |  |  |
| `GraphDefinition.useContext` | `GraphDefinition.useContext` | `useContext` |  | Context the content is intended to support | 0..* | UsageContext |  |  |
| `GraphDefinition.version` | `GraphDefinition.version` | `version` |  | Business version of the graph definition | 0..1 | string |  |  |
### Mapping Table
...
This group is centered on the Structure Definition GraphDefinition from hl7.fhir.r3.core@3.0.2 (R3, key 2).
All elements from this structure are listed while other structures only show contents that have relationships with those elements.

| *No Map* | Relationship | R3 GraphDefinition| Relationship | [R4 GraphDefinition](/docs/R4/Resources/GraphDefinition.md)| Relationship | [R4B GraphDefinition](/docs/R4B/Resources/GraphDefinition.md)| Relationship | [R5 GraphDefinition](/docs/R5/Resources/GraphDefinition.md)
| --- | --- | --- | --- | --- | --- | --- | --- | ---
...
| | | *47 of 47 elements used* | | *47 of 49 elements used* | | *47 of 49 elements used* | | *31 of 56 elements used* 
