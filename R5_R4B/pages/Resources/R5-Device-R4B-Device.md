...
| ------ | ------ | ------ | ------- |
| Device | Device | Equivalent | R5 `Device` maps as Equivalent to R4B `Device` |
...
| Device.conformsTo | - | DoesNotExistInTarget | R5 `Device.conformsTo` does not appear in the target and has no mapping for `Device`. |
...
| Device.contact | Device.contact | Equivalent | R5 `Device.contact` maps as Equivalent to R4B `Device.contact` |
| Device.contained | Device.contained | Equivalent | R5 `Device.contained` maps as Equivalent to R4B `Device.contained` |
| Device.cycle | - | DoesNotExistInTarget | R5 `Device.cycle` does not appear in the target and has no mapping for `Device`. |
| Device.definition | Device.definition | SourceIsBroaderThanTarget | R5 `Device.definition` maps as SourceIsBroaderThanTarget to R4B `Device.definition` - definition has change due to type change: R5 definition CodeableReference has no equivalent or mapped type in R4B definition |
...
| Device.expirationDate | Device.expirationDate | Equivalent | R5 `Device.expirationDate` maps as Equivalent to R4B `Device.expirationDate` |
| Device.extension | Device.extension | SourceIsBroaderThanTarget | R5 `Device.extension` maps as SourceIsBroaderThanTarget to R4B `Device.extension` - extension has change due to type change: R5 `extension` `Extension` maps as SourceIsBroaderThanTarget for R4B `extension` |
| Device.gateway | - | DoesNotExistInTarget | R5 `Device.gateway` does not appear in the target and has no mapping for `Device`. |
| Device.id | Device.id | Equivalent | R5 `Device.id` maps as Equivalent to R4B `Device.id` |
| Device.identifier | Device.identifier | Equivalent | R5 `Device.identifier` maps as Equivalent to R4B `Device.identifier` |
| Device.implicitRules | Device.implicitRules | Equivalent | R5 `Device.implicitRules` maps as Equivalent to R4B `Device.implicitRules` |
| Device.language | Device.language | RelatedTo | R5 `Device.language` maps as RelatedTo to R4B `Device.language` - language changed the binding strength from Required to Preferred |
| Device.location | Device.location | Equivalent | R5 `Device.location` maps as Equivalent to R4B `Device.location` |
...
| Device.udiCarrier.extension | Device.udiCarrier.extension | SourceIsBroaderThanTarget | R5 `Device.udiCarrier.extension` maps as SourceIsBroaderThanTarget to R4B `Device.udiCarrier.extension` - extension has change due to type change: R5 `extension` `Extension` maps as SourceIsBroaderThanTarget for R4B `extension` |
