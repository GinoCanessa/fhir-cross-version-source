...
| Target | http://hl7.org/fhir/ValueSet/event-status | EventStatus | EventStatus | Codes identifying the lifecycle stage of an event. |
...
| not-done | not-done | Equivalent | R5 `not-done` is equivalent to R4B `not-done`. |
| on-hold | on-hold | Equivalent | R5 `on-hold` is equivalent to R4B `on-hold`. |
| preparation | preparation | Equivalent | R5 `preparation` is equivalent to R4B `preparation`. |
